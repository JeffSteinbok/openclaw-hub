...
def update_ical_vevent(ical_data: str, **patches: str | dict | None) -> str:
    ...
    Args:
        ...
        **patches: Property names (uppercase) mapped to new string values, a
                   ``{"value": "...", "params": ";TZID=..."}`` patch dict, or
    ...
    def _format_patch_line(prop: str, patch: str | dict, existing_line: str | None = None) -> str:
        ...
        params = ""
        if existing_line:
            head = existing_line.split(":", 1)[0]
            if ";" in head:
                params = head[len(prop):]
        return f"{prop}{params}:{patch}"
    ...
    for line in unfolded.splitlines():
        ...
        if line == "END:VEVENT":
            ...
            for prop, val in patches.items():
                if prop not in handled_keys and val is not None:
                    out_lines.append(_format_patch_line(prop, val))
            out_lines.append(line)
            continue
        ...
        if prop_name in patches:
            ...
            if patches[prop_name] is not None:
                out_lines.append(_format_patch_line(prop_name, patches[prop_name], line))
