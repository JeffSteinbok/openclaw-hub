...
export function getTrackingUrl(trackingNumber, carrier) {
    const upper = trackingNumber.trim().toUpperCase();
...
export function scanTextForTrackingNumbers(text) {
    if (!text)
        return [];
...
export function removePackage(trackingNumber) {
    const upper = trackingNumber.trim().toUpperCase();
    if (!upper)
        return { error: "tracking_number is required" };
    const packages = loadPackages();
...
export function getPackage(trackingNumber) {
    const upper = trackingNumber.trim().toUpperCase();
    if (!upper)
        return { error: "tracking_number is required" };
    const packages = loadPackages();
...
export function isShippingSender(senderEmail) {
    if (!senderEmail)
        return false;
...
export const URL_EXTRACTION_RULES = [
    {
        name: "Narvar",
    ...
    {
        name: "FedEx",
        url_pattern: "https?://[^\\s\"'<>]*fedex\\.com/[^\\s\"'<>]*track[^\\s\"'<>]*",
        param_patterns: [
            "[?&]trknbr=(\\d{12,22})",
    ...
    {
        name: "Amazon",
        url_pattern: "https?://[^\\s\"'<>]*amazon\\.com/[^\\s\"'<>]*(?:track|order)[^\\s\"'<>]*",
        param_patterns: ["[?&]tracking-id=(TBA[0-9]{12}US)"],
        carrier_from_path: false,
    },
];
const NARVAR_CARRIER_PATH_MAP = {
    ups: "UPS",
    fedex: "FedEx",
    usps: "USPS",
    dhl: "DHL",
...
export function extractTrackingFromUrls(text) {
    if (!text)
        return [];
    const results = [];
    const seen = new Set();
...
function httpGet(url) {
    return new Promise((resolve, reject) => {
        ...
        const req = mod(url, {
            headers: {
                "User-Agent": "Mozilla/5.0 (compatible; OpenClaw/1.0; +https://github.com/JeffSteinbok/openclaw)",
            },
            timeout: 15_000,
        }, (res) => {
            let data = "";
...
export async function fetchNarvarTracking(url) {
