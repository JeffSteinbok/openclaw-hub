...
import { makeGuid, saveToAnalysis, writeMemoryForDate, updateState, } from "./memory.js";
import { buildNotificationPlan, routeAndNotify } from "./notify.js";
function detectDateFromHtml(htmlPath) {
    try {
        const html = readFileSync(htmlPath, "utf-8");
        const dayPattern = /(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday),?\s*(\w+ \d{1,2},?\s*\d{4})/;
...
function listScanImages(folder) {
    const images = [];
    let entries;
    try {
    ...
    for (const f of entries) {
        ...
        if (![".jpg", ".jpeg", ".png"].includes(ext))
            continue;
        if (f.startsWith("content-") || f === "body.html")
            continue;
        images.push(f);
    }
    return images;
 ...
 */
export async function processDigest(options) {
    const { folder, analysis = undefined, date = undefined, dryRun = false, visionBackend = "auto", messageId = undefined, persistAnalysis = true, writeMemory = true, sendNotifications = true, updateWorkflowState = true, workspaceAgent, memoryAgent, visionAgent, } = options;
    if (!workspaceAgent) {
    ...
    const notificationPlan = buildNotificationPlan(dateStr, Object.values(items), { workspaceAgent });
    let notifications = [];
    ...
    return {
        ...
        items: Object.entries(items).map(([fname, info]) => ({
            ...
            addressee: info.addressee ?? "Unknown",
            importance: info.importance ?? "unknown",
            description: info.description ?? "",
            guid: (info.guid ?? "").slice(0, 8),
        })),
